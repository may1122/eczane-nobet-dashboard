import osimport streamlit
as stimport pandas
as pdimport plotly.express 
as px
...
st.set_page_config(page_title="AYÇA | Eczane Nöbet Takip Sistemi",page_icon="💊",layout="wide")
...
def render_genel_hafta_ici_sonu(df, genel):
    ...
    <div class="card" style="margin-bottom:14px;">
        <div class="card-title">Grup bazlı hafta içi / cumartesi / pazar dağılımı</div>
        <div class="card-desc">
            Bu alan grup seçimine göre çalışır. Grafikte sadece Hafta İçi, Cumartesi ve Pazar gösterilir.
            Bayram ve arefe ayrı bir grafik unsuru olarak gösterilmez.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # Hafta sonunu toplam olarak tabloda tutuyoruz; grafikte ise Cumartesi ve Pazar ayrı gösteriliyor.
    ...
    if "Grup" not in kaynak.columns:
        st.error("Grup kolonu bulunamadı. Bu ekran için Grup bilgisi gereklidir.")
        return

    grup_listesi = sorted(kaynak["Grup"].dropna().unique())
    if not grup_listesi:
        st.warning("Gösterilecek grup bulunamadı.")
        return

    st.markdown('<div class="section-title">Grup Seçimi</div>', unsafe_allow_html=True)
    grup = st.selectbox("Grup seç", grup_listesi, key="genel_hafta_ici_sonu_grup")

    grup_kaynak = kaynak[kaynak["Grup"] == grup].copy()

    toplam_hafta_ici = int(grup_kaynak["Hafta İçi"].sum())
    toplam_cumartesi = int(grup_kaynak["Cumartesi"].sum())
    toplam_pazar = int(grup_kaynak["Pazar"].sum())
    ...
    st.markdown('<div class="section-title">Grafiksel Görünüm</div>', unsafe_allow_html=True)

    grafik_df = grup_kaynak[["Eczane", "Hafta İçi", "Cumartesi", "Pazar"]].copy()
    grafik_df = grafik_df.sort_values(["Hafta İçi", "Cumartesi", "Pazar"], ascending=[False, False, False])

    grafik_long = grafik_df.melt(
        id_vars=["Eczane"],
        value_vars=["Hafta İçi", "Cumartesi", "Pazar"],
        var_name="Tür",
    ...
    fig = px.bar(
        ...
        color="Tür",
        barmode="group",
        category_orders={"Tür": ["Hafta İçi", "Cumartesi", "Pazar"]},
        title=f"{grup} Grubu Hafta İçi / Cumartesi / Pazar Dağılımı",
        color_discrete_sequence=["#1f4b99", "#22a06b", "#f59f00"]
    )
    fig.update_layout(
        height=620,
    ...
    goster_kolonlari = [c for c in goster_kolonlari if c in grup_kaynak.columns]

    goster = grup_kaynak[goster_kolonlari].copy()
...
st.sidebar.markdown("### Bölüm")ana_bolum = st.sidebar.radio("",["Ana Panel","Detaylı Rapor"])
...
menu = st.sidebar.radio("",["Genel Özet","Tarih Seç","Aylık Takvim","Grup Analizi","Eczane Analizi","Genel Hafta İçi / Hafta Sonu"])

st.sidebar.markdown("---")st.sidebar.markdown("""<div class="small-note">Bu panel; nöbet dağılımını daha anlaşılır, daha düzenli ve daha kurumsal şekilde takip etmeniz için düzenlenmiştir.</div>""",unsafe_allow_html=True)
